...
/* Searches memory block that exactly matches nbytes from freelist.
 * Returns the memory address and updates freelist.
 * Returns NULL if the search fails. */
void *search_matched_freelist(unsigned int nbytes);

/* Searches any memory block that is larger than nbytes.
 * Returns the pointer to the reserved memory and updates freelist.
 * Returns NULL if memory cannot be allocated */
void *search_available_freelist(unsigned int nbytes);

/* Add a new pointer address to allocated_list. */
